...
| **5 — Cloud** (Sheet sync · push reminders/scheduler · e-invoice/e-way) | — | — | — | **UNBLOCKED** (GCP + Firebase now live in prod) — still unbuilt | ☐ owner-gated |

> **✅ SHIPPED + LIVE (2026-08).** Waves 1–4 all landed **and** post-Wave-4 work landed and **deployed to production** at https://opshub.gifsy.in: client credit-note capture, the PO **DRAFT→CONFIRMED→IN_PROGRESS** lifecycle, **confirm-invoice-WITHOUT-a-PO** + invoice **project attribution** (finance **P&L "Unattributed"** bucket). GCP/Firebase are live, so Wave 5's "blocked on GCP/Firebase" no longer holds — it remains owner-gated but unblocked and unbuilt. Gate at deploy: BE pytest **708** · FE vitest **175**; pushed to `github.com/NikunjSadani/Opshub` (`develop`).
