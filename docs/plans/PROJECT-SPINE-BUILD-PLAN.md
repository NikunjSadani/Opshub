...
| **4 — Action Center** (computed reminders: procurement T-15 · invoicing-due · AR-overdue) | 1 BE + 1 FE | audit + E2E | Single + UI/UX | done | ✅ BE+FE done; audit+E2E in progress. Read-only `action_center` module, computed on read (no tables/scheduler); reuses invoiced_qty rollup + ar_register(overdue). Gate BE pytest 561 · FE vitest 140. `1242101` (Stage 0) → `97af0ae` (BE+FE) |
