...
| **Env-parameterised harness; keep develop green** (`environments-topology`) | The E2E harness runs local (merge gate) — no `localhost`/fixed-auth assumptions baked in. **No auto-deploy** (updated 2026-08-31): `develop` runs CI only, so keep it green before pushing and a thorough local run should predict a clean prod `workflow_dispatch`. (A staging env is parked until portal acceptance.) |
...
- **Deploy rules (updated 2026-08-31):** dev / prod (a staging env is parked until portal acceptance). **No auto-deploy** — `develop` runs CI only (`ci.yml`, full `pytest`+`vitest`); keep it green before pushing. **Prod = manual `workflow_dispatch`** (Actions → "Deploy prod" → Run workflow — the deliberate click is the approval; a required-reviewer *pause* needs GitHub Pro/Team, unavailable on this free private plan), gated by the same `test` job, authenticating via **Workload Identity Federation** (no key files). Migrations run via an **in-VPC Alembic Cloud Run Job** (`--wait`, before the new revision serves). After any deploy, verify the **serving Cloud Run image SHA** matches the commit + `/health` → 200.
