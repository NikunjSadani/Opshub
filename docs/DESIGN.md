...
- **Upload template (inc 15 — LANDED, `28b627d`):** a wider, **structured** 26-column template — explicit `Challan Group`, split ship-to address (line1/line2/city/pincode, stored split & printed joined), inline consignee columns resolved by GSTIN, and the Project ID (validated Active + printed). **`brand` dropped.** Validation now emits non-blocking WARNINGS (consignee deviations, possible splits) alongside blocking errors, surfaced in the downloadable report (now `Row,Severity,Column,Problem`). Two owner decisions remain open (sparse-golden-record fill-in; retry-after-partial auto-reconcile) — see `RESUME.md`.
