...
**State — `develop` @ `e66a0c8`, UNPUSHED, gate-green** (BE: ruff 0 · mypy --strict 38 files · pytest 168 · `alembic check` no drift · FE: `tsc --noEmit` · `vite build` · vitest 24 · `terraform validate` clean). Stack: Python/FastAPI + React SPA (same-origin) · Firebase Auth (email/pw, not yet wired) · Postgres (sqlite local) · separate GCP project · DB Option-B · Cloud Run scale-to-zero · ~₹2k/mo.
