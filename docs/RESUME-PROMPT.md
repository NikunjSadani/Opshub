...
**State — `develop` @ `6bd6fe3`, UNPUSHED, gate-green** (BE: ruff 0 · mypy --strict 38 files · pytest 143 · `alembic check` no drift · FE: `tsc --noEmit` · `vite build` · vitest 8). Stack: Python/FastAPI + React SPA (same-origin) · Firebase Auth (email/pw, not yet wired) · Postgres (sqlite local) · separate GCP project · DB Option-B · Cloud Run scale-to-zero · ~₹2k/mo.
...
- **React frontend** (`frontend/src`): UI kit `ui/*` + `pages/challan/*` — New Challan (upload→generate→poll→download), Batches, Register (filters/e-way/admin Void), Master-Data CRUD. Built on `useApi()` (get/post/put/del/download/postForm) + react-query. **Auth is a MOCK admin** + a LOCAL-ONLY dev-auth shim (`DEV_AUTH=1`+`env=local`, double-guarded) so the SPA drives the real backend pre-Firebase.
