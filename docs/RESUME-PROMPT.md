...
**State — `develop` @ `6c38d2b`, UNPUSHED, gate-green** (BE: ruff 0 · mypy --strict 38 files · pytest 161 · `alembic check` no drift · FE: `tsc --noEmit` · `vite build` · vitest 14). Stack: Python/FastAPI + React SPA (same-origin) · Firebase Auth (email/pw, not yet wired) · Postgres (sqlite local) · separate GCP project · DB Option-B · Cloud Run scale-to-zero · ~₹2k/mo.
